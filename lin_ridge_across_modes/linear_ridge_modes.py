#
# For a given train_mode (Vocalized, Mimed, or Imagined):
#     a. Collects all sliding-window sEEG data + spectrogram targets for that modality (causal windows: [t-300ms, t]
#   → predict y[t]).
#     b. Splits by sentence (not by window) into train/held-out-test, so no sentence leaks across the split.
#     c. Collects the other two modalities in full as additional test sets.
#     d. Trains one Ridge decoder on the train_mode's training sentences.
#     e. Evaluates that single decoder on: its own held-out test, plus the full data of the other two modalities —
#   this is the actual "cross-modality generalization" test (e.g., train on Vocalized, test on Mimed/Imagined).
#     f. Has a --null flag to train a null model (shuffled targets) as a control, and --make_plots for quick envelope
#   diagnostics.
#     g. Saves one big .npz per train_mode (cross_modality_train_{mode}_with_heldout_test[_null].npz) containing
#   decoder weights, scalers, predictions, correlations, envelopes, metadata — this is the artifact everything
#   downstream reads.
...
def run_cross_modality_experiment(
    ...
    print("="*70)
    X_pool, y_pool, fi_pool = collect_all_data(wav_files, modality=train_mode, max_files=None)
    if len(X_pool) == 0:
    ...
    for mode in all_modes:
        ...
        print("="*70)
        X_m, y_m, fi_m = collect_all_data(wav_files, modality=mode, max_files=None)
        if len(X_m) == 0:
    ...
    if do_null:
        print("\n" + "="*70)
        print(f"TRAINING NULL MODEL (shuffled targets, seed={null_seed})")
        print("="*70)
        rng = np.random.default_rng(null_seed)
        perm = rng.permutation(len(y_train_norm))
        y_train_norm = y_train_norm[perm]   # shuffle targets, keep X fixed
    ...
    suffix = "_null" if do_null else ""
    out_file = os.path.join(
        OUT_DIR,
        f"cross_modality_train_{train_mode_l}_with_heldout_test{suffix}.npz"
    )
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(description="Cross-modality stimulus reconstruction with optional null model")

    ap.add_argument(
        "--train_mode",
        type=str,
        default="ALL",
        choices=["Vocalized", "Mimed", "Imagined", "ALL"],
        help="Which modality to train on (default: ALL)"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="If 1, train a null model by shuffling training targets"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=2026,
        help="Random seed for null shuffling"
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=1,
        help="Whether to make quick envelope plots (1=yes, 0=no)"
    )

    args = ap.parse_args()

    train_modes = (
        ["Vocalized", "Mimed", "Imagined"]
        if args.train_mode == "ALL"
        else [args.train_mode]
    )

    for tm in train_modes:
        run_cross_modality_experiment(
            train_mode=tm,
            make_plots=bool(args.make_plots),
            do_null=bool(args.null),
